...
def test_process_file_writes_conformant_okf_doc(
    bundle_root: Path, app_root: Path, config: Config
) -> None:
    source = app_root / "_inbox" / "My Test Note.txt"
...
def test_process_file_updates_indexes_and_log(
    bundle_root: Path, app_root: Path, config: Config
) -> None:
    source = app_root / "_inbox" / "note.txt"
...
def test_process_file_commits_to_git(repo_root: Path, app_root: Path, config: Config) -> None:
    import subprocess

    from okeef.git_ops import _git_executable

    source = app_root / "_inbox" / "committed-note.txt"
...
def test_duplicate_titles_get_unique_filenames(
    bundle_root: Path, app_root: Path, config: Config
) -> None:
    for i in range(2):
        source = app_root / "_inbox" / f"dup{i}.txt"
        source.write_text("Same Title\ncontent", encoding="utf-8")
        # force identical classification output by reusing the same filename stem
        source = source.rename(app_root / "_inbox" / "dup.txt")
        pipeline.process_file(source, config)
